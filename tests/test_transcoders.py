...
class JSONTests(unittest.TestCase):
    ...
    def test_that_uuids_are_dumped_as_strings(self):
        obj = {'id': uuid.uuid4()}
        dumped = self.transcoder.marshall(obj)
    ...
    def test_that_bytes_are_base64_encoded(self):
        ...
        dumped = self.transcoder.marshall({'bin': bin})
        self.assertEqual(
    ...
    def test_that_unhandled_objects_raise_type_error(self):
        with self.assertRaises(TypeError):
            self.transcoder.marshall(object())
...
class MessagePackTests(unittest.TestCase):
    ...
    def test_that_strings_are_dumped_as_strings(self):
        dumped = self.transcoder.marshall(u'foo')
        self.assertEqual(self.transcoder.unmarshall(dumped), 'foo')
        self.assertEqual(dumped, pack_string('foo'))

    def test_that_none_is_packed_as_nil_byte(self):
        self.assertEqual(self.transcoder.marshall(None), b'\xC0')

    def test_that_bools_are_dumped_appropriately(self):
        self.assertEqual(self.transcoder.marshall(False), b'\xC2')
        self.assertEqual(self.transcoder.marshall(True), b'\xC3')

    def test_that_ints_are_packed_appropriately(self):
        self.assertEqual(self.transcoder.marshall((2 ** 7) - 1), b'\x7F')
        self.assertEqual(self.transcoder.marshall(2 ** 7), b'\xCC\x80')
        self.assertEqual(self.transcoder.marshall(2 ** 8), b'\xCD\x01\x00')
        self.assertEqual(self.transcoder.marshall(2 ** 16),
                         b'\xCE\x00\x01\x00\x00')
        self.assertEqual(self.transcoder.marshall(2 ** 32),
                         b'\xCF\x00\x00\x00\x01\x00\x00\x00\x00')

    def test_that_negative_ints_are_packed_accordingly(self):
        self.assertEqual(self.transcoder.marshall(-(2 ** 0)), b'\xFF')
        self.assertEqual(self.transcoder.marshall(-(2 ** 5)), b'\xE0')
        self.assertEqual(self.transcoder.marshall(-(2 ** 7)), b'\xD0\x80')
        self.assertEqual(self.transcoder.marshall(-(2 ** 15)), b'\xD1\x80\x00')
        self.assertEqual(self.transcoder.marshall(-(2 ** 31)),
                         b'\xD2\x80\x00\x00\x00')
        self.assertEqual(self.transcoder.marshall(-(2 ** 63)),
                         b'\xD3\x80\x00\x00\x00\x00\x00\x00\x00')

    def test_that_lists_are_treated_as_arrays(self):
        dumped = self.transcoder.marshall(list())
        self.assertEqual(self.transcoder.unmarshall(dumped), [])
        self.assertEqual(dumped, b'\x90')

    def test_that_tuples_are_treated_as_arrays(self):
        dumped = self.transcoder.marshall(tuple())
        self.assertEqual(self.transcoder.unmarshall(dumped), [])
        self.assertEqual(dumped, b'\x90')

    def test_that_sets_are_treated_as_arrays(self):
        dumped = self.transcoder.marshall(set())
        self.assertEqual(self.transcoder.unmarshall(dumped), [])
        self.assertEqual(dumped, b'\x90')
    ...
    def test_that_uuids_are_dumped_as_strings(self):
        uid = uuid.uuid4()
        dumped = self.transcoder.marshall(uid)
        self.assertEqual(self.transcoder.unmarshall(dumped), str(uid))
        self.assertEqual(dumped, pack_string(uid))

    def test_that_datetimes_are_dumped_in_isoformat(self):
        now = datetime.datetime.now()
        dumped = self.transcoder.marshall(now)
        self.assertEqual(self.transcoder.unmarshall(dumped), now.isoformat())
    ...
    def test_that_bytes_are_sent_as_bytes(self):
        ...
        dumped = self.transcoder.marshall(data)
        self.assertEqual(self.transcoder.unmarshall(dumped), data)
